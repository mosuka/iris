...
//!
//! ```
//! use iris::analysis::tokenizer::lindera::LinderaTokenizer;
...
impl LinderaTokenizer {
    ...
    pub fn new(mode_str: &str, dict_uri: &str, user_dict_uri: Option<&str>) -> Result<Self> {
        let mode = Mode::from_str(mode_str)
            .map_err(|e| IrisError::analysis(format!("Invalid mode '{}': {}", mode_str, e)))?;
        let dict = load_dictionary(dict_uri)
            .map_err(|e| IrisError::analysis(format!("Failed to load dictionary: {}", e)))?;
        let metadata = &dict.metadata;
        let user_dict = match user_dict_uri {
            Some(uri) => Some(load_user_dictionary(uri, metadata).map_err(|e| {
                IrisError::analysis(format!("Failed to load user dictionary: {}", e))
            })?),
            None => None,
        };
...
impl Tokenizer for LinderaTokenizer {
    fn tokenize(&self, text: &str) -> Result<TokenStream> {
        ...
        for token in self
            ...
            .map_err(|e| IrisError::analysis(format!("Failed to segment text: {}", e)))?
        {
