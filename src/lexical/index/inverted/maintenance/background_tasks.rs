...
impl BackgroundScheduler {
    ...
    pub fn start(&self) -> Result<()> {
        if self.running.load(Ordering::Acquire) {
            return Err(IrisError::index("Background scheduler already running"));
        }
    ...
    pub fn submit_task(&self, task: BackgroundTask) -> Result<()> {
        if !self.running.load(Ordering::Acquire) {
            return Err(IrisError::index("Background scheduler not running"));
        }
        ...
        self.task_sender
            .send(task)
            .map_err(|_| IrisError::index("Failed to submit task to queue"))?;
