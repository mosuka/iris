...
Laurus は Rust で書かれたコンポーザブルな検索コアライブラリです — Lexical Augmented Unified Retrieval Using Semantics のために構築されています。
モノリシックなエンジンではなく、あらゆるアプリケーションに強力な検索を組み込むためのモジュール式ビルディングブロックを提供します:

- **Lexical 検索プリミティブ** — 正確な完全一致検索
- **ベクトルベースの類似度検索** — 深い意味理解
- **ハイブリッドスコアリングとランキング** — 複数のシグナルを統合した一貫性のある結果

モノリシックな検索エンジンとして機能するのではなく、Laurus は**コンポーザブルな検索コア**として設計されています — アプリケーションへの組み込み、カスタムロジックによる拡張、分散システム内でのオーケストレーションを想定したモジュール式ビルディングブロック群です。
